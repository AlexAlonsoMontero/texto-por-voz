...
export class HybridSafeAreaService implements ISafeAreaService {
  ...
  /**
   * Método 2: Diferencias entre screen y viewport
   */
  private getViewportDifferenceInsets(): SafeAreaInsets {
    ...
    return {
      top: heightDiff > 20 ? Math.min(heightDiff * 0.35, 30) : 0,  // Menos peso arriba
      bottom: heightDiff > 20 ? Math.min(heightDiff * 0.65, 50) : 0, // Menos peso abajo
      left: 0,
      right: 0,
    };
  }
  ...
  private getHeuristicInsets(): SafeAreaInsets {
    ...
    if (isAndroid) {
      return {
        top: 20,  // Status bar más conservadora
        bottom: 36, // Navigation bar más conservadora (típicamente 36-48px)
        left: 0,
        right: 0
      };
    }
...
export class HybridSafeAreaService implements ISafeAreaService {
  ...
  private getHeuristicInsets(): SafeAreaInsets {
    ...
    if (isAndroid) {
      return {
        top: 24, // Status bar típica en Android
        bottom: 48, // Navigation bar típica
        left: 0,
        right: 0,
      };
    }
